...
namespace Microsoft.AspNet.Mvc.Razor
{
    public class CompilerCacheTest
    {
        ...
        public void GetOrAddMetadata_ReturnsValueFromValueFactoryIfCacheEntryDoesNotExist()
        {
            ...
            var result = CompilationResult.Successful(typeof(PreCompile));

            // Act
            var actual1 = cache.GetOrAddMetadata(relativeFileInfo,
                                                _ => result,
                                                key,
                                                () => expected);

            var actual2 = cache.GetOrAddMetadata(relativeFileInfo,
        ...
        public static IEnumerable<object[]> GetOrAddMetadata_ReturnsValueFromValueFactoryIfCacheEntryIsInvalidData
        {
            get
            {
                ...
                var originalFileInfo = new TestFileInfo
                {
                    Content = "content",
                    LastModified = lastModified,
                };
                ...
                // Length does not match
                var modified1 = new TestFileInfo
                {
                    Content = "different-content",
                    LastModified = lastModified
                };

                yield return new[] { originalFileInfo, modified1 };

                // Length is same but content and last modified do not match
                var modified2 = new TestFileInfo
                {
                    Content = "c0ntent",
                    LastModified = lastModified.AddSeconds(100),
                };

                yield return new object[] { originalFileInfo, modified2 };
            }
        }
        ...
        public void GetOrAddMetadata_ReturnsValueFromValueFactoryIfCacheEntryIsInvalid(
            TestFileInfo originalFileInfo, TestFileInfo modifiedFileInfo)
        {
            // Arrange
            ...
            var relativeFileInfo = new RelativeFileInfo(originalFileInfo, path);
            var compilationResult = CompilationResult.Successful(typeof(PreCompile));

            // Act - 1
            var result1 = cache.GetOrAddMetadata(relativeFileInfo,
                                                _ => compilationResult,
                                                key,
                                                () => expected1);
            ...
            // Act - 2
            var result2 = cache.GetOrAddMetadata(new RelativeFileInfo(modifiedFileInfo, path),
                                                _ => compilationResult,
                                                key,
                                                () => expected2);
        ...
        [Fact]
        public void GetOrAddMetadata_ReturnsValueFromValueFactoryIfAssociatedViewStartChanges()
        {
            // Arrange
            var path = "views\\Home\\Index.cshtml";
            var fileSystem = new TestFileSystem();
            ...
            var expected1 = new object();
            var expected2 = new object();
            var relativeFileInfo = new RelativeFileInfo(fileInfo, path);
            var compilationResult = CompilationResult.Successful(typeof(PreCompile));
            ...
            // Act - 2
            fileSystem.AddFile("views\\_viewstart.cshtml", "");
            var result2 = cache.GetOrAddMetadata(new RelativeFileInfo(fileInfo, path),
                                                _ => compilationResult,
                                                key,
                                                () => expected2);
        ...
        public void GetOrAddMetadata_ReturnsCachedValueIfCacheEntryIsValid()
        {
            ...
            // Act - 2
            var result2 = cache.GetOrAddMetadata(relativeFileInfo,
